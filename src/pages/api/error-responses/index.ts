...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getErrorResponses() {
    const {
      limit: _limit,
      offset: _offset,
      order,
    ...
    const response = await prisma.error_response
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'error_response'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createErrorResponse() {
    ...
    const data = await prisma.error_response.create({
      data: body,
    });
