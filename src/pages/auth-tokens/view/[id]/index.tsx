...
import { getAuthTokenById } from 'apiSdk/auth-tokens';
import { AuthTokenInterface } from 'interfaces/auth-token';

function AuthTokenViewPage() {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('auth_token', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/auth-tokens/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Jwt Token" text={data?.jwt_token} />

                <FormListItem
                  label="Expires At"
                  text={data?.expires_at ? format(parseISO(data?.expires_at as unknown as string), 'dd-MM-yyyy') : ''}
                />
