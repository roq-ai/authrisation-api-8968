...
import { getAuthTokens } from 'apiSdk/auth-tokens';
import { AuthTokenInterface } from 'interfaces/auth-token';
import { getProfiles } from 'apiSdk/profiles';
import { ProfileInterface } from 'interfaces/profile';
import { getErrorResponses } from 'apiSdk/error-responses';
import { ErrorResponseInterface } from 'interfaces/error-response';

import { PasswordManagementListPage as ShowCaseList } from 'pages/password-managements';

function DashboardPage() {
  const { session } = useSession();

  const authTokenStat = useSWR<PaginatedInterface<AuthTokenInterface>>(
    () => `/auth-tokens`,
    () => getAuthTokens({ limit: 1, offset: 0 }),
  );
  ...
  const errorResponseStat = useSWR<PaginatedInterface<ErrorResponseInterface>>(
    () => `/error-responses`,
    () => getErrorResponses({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Auth Tokens',
      result: authTokenStat?.data?.totalCount,
      isLoading: authTokenStat.isLoading,
    ...
    },
    {
      ...
      result: errorResponseStat?.data?.totalCount,
      isLoading: errorResponseStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Password Managements
                </Text>
                <NextLink href={`password-managements`} passHref legacyBehavior>
                  <Button
