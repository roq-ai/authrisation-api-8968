...
import { createErrorResponse } from 'apiSdk/error-responses';
...
function ErrorResponseCreatePage() {
  ...
  const handleSubmit = async (values: ErrorResponseInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createErrorResponse(values);
      resetForm();
