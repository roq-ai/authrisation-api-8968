...
import { getPasswordManagementById, updatePasswordManagementById } from 'apiSdk/password-managements';
...
function PasswordManagementEditPage() {
  ...
  const handleSubmit = async (values: PasswordManagementInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePasswordManagementById(id, values);
      mutate(updated);
      resetForm();
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Password Management',
              isCurrent: true,
            },
          ]}
        />
